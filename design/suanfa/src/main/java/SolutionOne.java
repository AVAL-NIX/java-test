...
public class SolutionOne {
    ...
    static class SetAssociativeCacheRunner {
        public static void parseInput(InputStream inputStream) throws IOException {
            ...
            // todo
            IReplacementAlgo iReplacementAlgo = null;
...
//                    cache = createCache(line, replacementAlgoName);
                    // todo
                    iReplacementAlgo = new ReplacementAlgoFactory().createReplacementAlgo(line);
...
//                    Object retValue = SetAssociativeCacheFactory.InvokeCacheMethod(line, cache);
                    //todo
                    Object retValue = SetAssociativeCacheFactory.InvokeCacheMethodByInterface(line, iReplacementAlgo);
    ...
    interface IReplacementAlgo {
        ...
        public IReplacementAlgo init(long count, long size);
    ...
    public static class SetAssociativeCacheFactory {
        ...
        /// NOTE: Modify only if you change the main interface of SetAssociativeCache
        public static Object InvokeCacheMethodByInterface(String inputLine, IReplacementAlgo cacheInstance) {
            String[] callArgs = Arrays.stream(inputLine.split(",", -1)).map(a -> a.trim()).toArray(n -> new String[n]);
    ...
    public static class ReplacementAlgoFactory {
        IReplacementAlgo createReplacementAlgo(String inputLine) {
            // 1. switch
            // 2. 还可以用JAVA注解干掉switch 。
            // 3. 还可以用反射 来装 IReplacementAlgo 干掉 swithc
            String[] cacheParams = Arrays.stream(inputLine.split(",")).map(s -> s.trim()).toArray(n -> new String[n]);
            int setCount = Integer.parseInt(cacheParams[0]);
            int setSize = Integer.parseInt(cacheParams[1]);
...
//                case LruAlgorithm:
//                    return new LRUReplacementAlgo().init(setCount, setSize);
//                case MruAlgorithm:
//                    return new MRUReplacementAlgo().init(setCount, setSize);
//                default:
...
//            }
            // todo 2
            // 10,10,MRUReplacementAlgo
            try {
                Class<?> aClass = Class.forName(SolutionOne.class.getSimpleName() + "$" + replacementAlgoName);
                return (IReplacementAlgo) aClass.newInstance();
            } catch (Exception e) {
                e.printStackTrace();
            }
